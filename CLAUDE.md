...
- Shared entrypoint `/srv/edge`: Caddy on 80/443 with automatic Let's Encrypt certificates, importing one file per project from `sites/`. Adding a project never edits a shared file. **[deploy/Caddyfile](deploy/Caddyfile) is the only deployed file no pipeline applies** - the server's `sites/cooking-assistant.caddy` is edited by hand and reloaded with `caddy reload`. Changing it is therefore a required manual step of that release, and one nothing catches: both container health checks run inside the project network, so a proxy pointing at the wrong port reports a fully green deploy while every visitor gets a 502.
...
Routing is the folder tree under [frontend/src/app/](frontend/src/app/). Three route groups carry the whole map and never appear in a URL: `(auth)` for the sign-in pages, `(public)` for everything a guest may read, and `(private)`, whose `layout.tsx` is a single `PrivateRoute` wrapper - **a page is private because of where it lives, so it cannot forget its own guard**. **`page.tsx` is the page** - the component lives there, with its own stylesheet and its co-located `__tests__/`, so a route is one folder and there is no parallel tree mirroring it. The only exception is [frontend/src/views/](frontend/src/views/), which still holds the five public pages (`/`, both listings, both detail pages) because they are about to be rewritten as server components; the folder goes away with them. The root `layout.tsx` owns `<html>`/`<body>` and the providers, `loading.tsx` is the one Suspense boundary every route gets (and what lets pages read search params), `error.tsx` and `not-found.tsx` replace the old router-level error and 404 elements - an unknown URL now answers with a real HTTP 404. `constants/routes.ts` is no longer the router's source of truth but is still the only place a path may be written: its builders produce every link, and `PUBLIC_PATHS` feeds `matchRoutePattern` in the api layer. **The app has exactly one `route.ts`** - `app/health/route.ts`, the container's liveness probe. Everything the product itself serves over HTTP belongs to the backend and is reached through the `api/` layer; do not add a second route handler to move application logic into the frontend process.
...
Page components live in their own route folder under [frontend/src/app/](frontend/src/app/), as `page.tsx`. The five public pages still waiting to become server components are the exception and stay under [frontend/src/views/](frontend/src/views/) for now - a folder deliberately not called `pages`, which Next reserves for the legacy Pages Router and refuses to have in a different directory from `app/`. Reusable UI lives in [frontend/src/components/](frontend/src/components/), grouped by domain plus `layout/`, `ui/`, and `icons/`. The stats page renders its charts with `recharts` (lazy-loaded page chunk); PDF export was removed in 3.3. Recipe types are reference data behind a read-only API - there is no recipe-types page or component folder (the CRUD page was removed in 1.40).
