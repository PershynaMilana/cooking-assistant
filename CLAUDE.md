...
State lives in custom hooks under [frontend/src/hooks/](frontend/src/hooks/) (50+, composed) and in a Redux Toolkit store under [frontend/src/store/](frontend/src/store/). The store is wired in [frontend/src/store/store.ts](frontend/src/store/store.ts); typed hooks (`useAppDispatch`, `useAppSelector`) live in [frontend/src/store/hooks.ts](frontend/src/store/hooks.ts). Each domain slice lives in `redux/slices/<domain>Slice.ts`; its selectors always live in a separate `redux/selectors/<domain>Selectors.ts` - never inline in the slice or in components. User-facing strings go through i18next ([frontend/src/i18n/](frontend/src/i18n/), one namespace per domain, `en` only) via `useTranslation`. Bare path aliases (`api/`, `hooks/`, `components/`, `pages/`, `types/`, `utils/`, `constants/`, `config/`, `i18n/`, `redux/`) - never `../` across folders.
