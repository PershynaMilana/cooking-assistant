...
const PurchaseHistoryModal: React.FC<PurchaseHistoryModalProps> = ({
  ...
  const saveChanges = async (id: number, newQuantity: number) => {
    ...
    try {
      const userId = jwtDecode<{ id: number }>(token).id;
      console.log( `http://localhost:8080/api/user-ingredients/${userId}/history/${id}`, newQuantity, `Bearer ${token}`)
      await axios.put(
  ...
  const isPurchaseExpired = (purchaseDate: string, daysToExpire: number): boolean => {

    console.log(purchaseDate, daysToExpire)
    if (!purchaseDate || !daysToExpire) {
      return false; // если данных нет, считаем не просроченной
    }
    ...
    return today >= expirationDate; // если сегодняшняя дата больше даты истечения
  };
          ...
          {!loading && !error && purchaseHistory.length > 0 && (
              <ul className="space-y-2">
                {purchaseHistory.map((purchase) => {
                  const expired = isPurchaseExpired(purchase.purchase_date, purchase.days_to_expire);
                  return (
                      <li
                          key={purchase.id}
                          className={`flex justify-between items-center p-2 rounded ${expired ? "bg-red-100" : "bg-gray-100"}`}
                      >
                        ...
                        <input
                            ...
                            onChange={(e) => handleQuantityChange(purchase.id, +e.target.value)}
                            onBlur={(e) => saveChanges(purchase.id, +e.target.value)}
                        />
